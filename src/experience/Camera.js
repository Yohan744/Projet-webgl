...
export default class Camera {
    constructor() {
        ...
        this.basicCameraPosition = new THREE.Vector3(0.05, 2.25, -4.5);
        this.basicLookingPoint = new THREE.Vector3(0, -0.25, -25);
    ...
    setKeyListener() {
        window.addEventListener('keydown', (event) => {
            if (event.key === 'a') {
                this.moveCameraZ(new THREE.Vector3(this.modes.default.instance.position.x, this.modes.default.instance.position.y, 12));
            }
        });
    }

    moveCameraZ(targetPosition, duration = 10) {
        if (this.isMoving) return;

        this.isMoving = true;
        const distanceToPoint = Math.round(targetPosition.distanceTo(this.modes.default.instance.position));
